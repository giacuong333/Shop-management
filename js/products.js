...
const searchProductName = (searchName) => {
    const productList = document.querySelector("#products-area")
    productList.innerHTML = ""
    let productHTML = ""

    // Return an array contains product filtered 
    const filteredProducts = products.filter((product) => product.title.toLowerCase().startsWith(searchName.toLowerCase()))
    ...
    filteredProducts.forEach((productItem) => {
        const { id, title, price, description, image, sale, details } = productItem
        productHTML += `
        <div onclick="productsDetails(${id})" class="col l-6 m-6 c-12 product-details">
            <a href="${details}" target="_self" class="product product--space ${sale}">
              <img src="${image}" alt="Image" class="product-img"/>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Render products when users clicked to the search icon 
    ...
    // Render products when users pressed enter
